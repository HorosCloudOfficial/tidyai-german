...
function Invoke-OpenAIRequest {
    ...
    if ([string]::IsNullOrEmpty($OPENAI_API_KEY)) {
        Write-ColorText "Groq API key nicht gefunden!" $Colors.Error
        return $null
    }
...
"@
    ...
    if ($requestSize -gt 100000) {  # 100KB limit
        Write-ColorText "WARNUNG: Große Anfragegröße kann zu Abschneidung führen" $Colors.Warning
        Write-ColorText "Erwäge, die Anzahl der Dateien zu reduzieren oder die Anfrage zu vereinfachen" $Colors.Info
    }
    ...
    try {
        ...
        if ($finishReason -eq "length") {
            Write-ColorText "FEHLER: Antwort wurde aufgrund des Token-Limits abgeschnitten!" $Colors.Error
            Write-ColorText "Die Antwort wurde abgebrochen. Bitte versuche es mit weniger Dateien oder kontaktiere den Support." $Colors.Warning
        ...
        Write-Host ""
        Write-ColorText "Fehler bei Kommunikation mit Groq API: $($_.Exception.Message)" $Colors.Error
        ...
        try {
            if ($_.Exception.Response) {
                ...
                if ($errorDetails) {
                    Write-ColorText "API-Fehlerdetails: $errorDetails" $Colors.Warning
                } else {
                    Write-ColorText "Keine zusätzlichen Fehlerdetails verfügbar" $Colors.Warning
                }
            }
        }
        catch {
            Write-ColorText "Konnte detaillierte Fehlerinformationen nicht abrufen" $Colors.Warning
        }
