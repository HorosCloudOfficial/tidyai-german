...
function ConvertFrom-AIResponse {
    ...
    try {
        ...
        Write-ColorText "Fehler bei Verarbeitung der KI-Antwort: $($_.Exception.Message)" $Colors.Error
        return $null
    }
}
...
function Get-JsonFromResponse {
    ...
    if ($cleaned -match '```json\s*\n([\s\S]*?)\n\s*```') {
        Write-ColorText "JSON in Markdown-Codeblock gefunden" $Colors.Success
        return $matches[1].Trim()
    }
    
    # Pattern 2: JSON in generic markdown code block
    if ($cleaned -match '```\s*\n(\[[\s\S]*?\])\s*\n```') {
        Write-ColorText "JSON-Array in Codeblock gefunden" $Colors.Success
        return $matches[1].Trim()
    }
    ...
    if ($cleaned.StartsWith('```')) {
        ...
        if ($cleaned -match '(\[[\s\S]*?\])') {
            Write-ColorText "JSON-Array nach Markdown-Bereinigung gefunden" $Colors.Success
            return $matches[1].Trim()
        }
    }
    
    # If no patterns match, return the cleaned response as-is
    Write-ColorText "Kein spezifisches JSON-Muster gefunden, verwende bereinigte Antwort" $Colors.Warning
    return $cleaned
}
...
function ConvertFrom-JsonResponse {
    ...
    if ($trimmedResponse -eq "[]" -or $trimmedResponse -eq "") {
        Write-ColorText "FEHLER: KI hat leere Antwort zurückgegeben" $Colors.Error
    ...
    if (-not $JsonString.Contains('folderName')) {
        Write-ColorText "FEHLER: Antwort fehlt erforderliche Struktur (folderName)" $Colors.Error
    ...
    try {
        ...
        Write-ColorText "$($folders.Count) Ordner erfolgreich aus KI-Antwort geparst" $Colors.Success
        ...
        Write-ColorText "JSON-Parsing fehlgeschlagen: $($_.Exception.Message)" $Colors.Error
        
        # Check if response might be truncated
        $isTruncated = Test-JsonTruncation -JsonString $JsonString
        if ($isTruncated) {
            Write-ColorText "FEHLER: Antwort scheint abgeschnitten zu sein!" $Colors.Error
            Write-ColorText "TidyAI benötigt vollständige Antworten. Bitte versuche es erneut." $Colors.Warning
