...
function Scan-Folder {
    ...
    Write-ColorText "Scanne Ordner: $Path" $Colors.Info
    Write-Host ""
    ...
    try {
        ...
        Write-Host ""
        Write-ColorText "$totalItems Elemente erfolgreich gescannt!" $Colors.Success
        Write-Host ""
        
        return $items
    }
    catch {
        Write-ColorText "Fehler beim Scannen des Ordners: $($_.Exception.Message)" $Colors.Error
        return @()
    }
}
